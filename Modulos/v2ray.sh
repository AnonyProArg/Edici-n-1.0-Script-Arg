...
source <(curl -sL https://raw.githubusercontent.com/AnonyProArg/Edici-n-1.0-Script-Arg/main/Install/ArchivosUtilitarios/V2RAY/install-v2ray.sh)
msg -ama "$(fun_trans "Intalado con Exito")!"
USRdatabase="/etc/VPS-ARG/RegV2ray"
...
unistallv2 () {
source <(curl -sL https://raw.githubusercontent.com/AnonyProArg/Edici-n-1.0-Script-Arg/main/Install/ArchivosUtilitarios/V2RAY/install-v2ray.sh) --remove > /dev/null 2>&1
rm -rf /etc/VPS-ARG/RegV2ray > /dev/null 2>&1
...
##Registro
echo "  $UUID | $nick | $valid " >> /etc/VPS-ARG/RegV2ray
Fecha=`date +%d-%m-%y-%R`
cp /etc/VPS-ARG/RegV2ray /etc/VPS-ARG/v2ray/RegV2ray-"$Fecha"
v2ray restart > /dev/null 2>&1
echo ""
v2ray info > /etc/VPS-ARG/v2ray/confuuid.log
lineP=$(sed -n '/'${UUID}'/=' /etc/VPS-ARG/v2ray/confuuid.log)
numl1=4
let suma=$lineP+$numl1
sed -n ${suma}p /etc/VPS-ARG/v2ray/confuuid.log 
echo ""
msg -bar
...
echo -e "\e[33m$(cat /etc/VPS-ARG/RegV2ray|cut -d '|' -f2,1)" 
msg -bar
...
linePre=$(sed -n '/'${uuidel}'/=' /etc/VPS-ARG/RegV2ray)
sed -i "${linePre}d" /etc/VPS-ARG/RegV2ray
numl1=2
...
msg -bar
# usersss=$(cat /etc/VPS-ARG/RegV2ray|cut -d '|' -f1)
# cat /etc/VPS-ARG/RegV2ray|cut -d'|' -f3
VPSsec=$(date +%s)
local HOST="/etc/VPS-ARG/RegV2ray"
local HOST2="/etc/VPS-ARG/RegV2ray"
...
DateExp="$(cat /etc/VPS-ARG/RegV2ray|grep -w "$hostreturn"|cut -d'|' -f3)"
if [[ ! -z $DateExp ]]; then             
...
usris="$(cat /etc/VPS-ARG/RegV2ray|grep -w "$hostreturn"|cut -d'|' -f2)"
...
linesss=$(cat /etc/VPS-ARG/RegV2ray | wc -l)
...
estarts () {
VPSsec=$(date +%s)
local HOST="/etc/VPS-ARG/v2ray/lisportt.log"
local HOST2="/etc/VPS-ARG/v2ray/lisportt.log"
local RETURN="$(cat $HOST|cut -d'|' -f2)"
local IDEUUID="$(cat $HOST|cut -d'|' -f1)"
if [[ -z $RETURN ]]; then
...
i=1
while read hostreturn ; do
iptables -n -v -L > /etc/VPS-ARG/v2ray/data1.log 
statsss=$(cat /etc/VPS-ARG/v2ray/data1.log|grep -w "tcp spt:$hostreturn quota:"|cut -d' ' -f3,4,5)
gblim=$(cat /etc/VPS-ARG/v2ray/lisportt.log|grep -w "$hostreturn"|cut -d'|' -f2)
...
[[ ! -z $contador_secuencial ]] && {
linesss=$(cat /etc/VPS-ARG/v2ray/lisportt.log | wc -l)
...
iptables-save > /etc/iptables/rules.v4
lineP=$(sed -n '/'${portbg}'/=' /etc/VPS-ARG/v2ray/lisportt.log)
sed -i "${linePre}d" /etc/VPS-ARG/v2ray/lisportt.log
msg -bar
msg -ne "Enter Para Continuar" && read enter
${SCPinst}/v2ray.sh 
}
